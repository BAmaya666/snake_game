...
class FRUIT:
	...
	def draw_fruit(self):
		...
		screen.blit(apple,fruit_rect)	# pygame.draw.rect(screen,(126,166,114),fruit_rect)
...
class MAIN:
	...
	def check_fail(self):
		if not 0 <= self.snake.body[0].x < cell_number or not 0 <= self.snake.body[0].y < cell_number:	# check if snake is outside of the screen
			self.game_over()
	...
	def draw_score(self):
		...
		pygame.draw.rect(screen,(0,0,102),bg_rect)
		screen.blit(score_surface,score_rect)
		screen.blit(apple,apple_rect)
		pygame.draw.rect(screen,(178,255,102),bg_rect,2)
...
while True:
	for event in pygame.event.get(): #Inside here we're going to draw all the elements
		if event.type == pygame.QUIT: #if we close the window
			pygame.quit()
			sys.exit()
		if event.type == SCREEN_UPDATE:
			main_game.update()
		if event.type == pygame.KEYDOWN:
			if event.key ==pygame.K_KP5: #Up
				if main_game.snake.direction.y != 1:
					main_game.snake.direction = Vector2(0,-1)
			if event.key ==pygame.K_KP2: #Down
				if main_game.snake.direction.y != -1:
					main_game.snake.direction = Vector2(0,1)
			if event.key ==pygame.K_KP1:#Left
